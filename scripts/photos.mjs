...
async function meals() {
  ...
  for (const q of ['pizza']) {
    const j = await (await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?s=${q}`)).json();
...
async function mealThumb(nameSub) {
  ...
  const key = Object.keys(idx).find((k) => k.toLowerCase().includes(nameSub.toLowerCase()));
...
async function djProduct(category, titleSub) {
  if (!djCache.has(category)) {
    const j = await (
      await fetch(`https://dummyjson.com/products/category/${category}?limit=30&select=title,thumbnail`)
    ).json();
...
export const SOURCES = {
  ...
  menu_hero:   { get: () => uns('1565299624946-b28f40a0ae38', 1200, 820), box: [1200, 820] }, // moody whole pizza
...
export async function processed(token) {
  ...
  return sharp(buf).resize(W, H, { fit: 'cover', position: 'attention' }).jpeg({ quality: 80 }).toBuffer();
}
