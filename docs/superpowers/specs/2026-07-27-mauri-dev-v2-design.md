...
### Found during v2 (numbering continues from the original audit)

| #   | Location                                    | Issue                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| --- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| B20 | `src/components/Reveal.tsx` + `globals.css` | With JavaScript disabled every `.reveal` stayed at `opacity: 0`, so the page rendered blank — content in the HTML, invisible to reader and to any crawler that does not execute JS. `ProjectsGrid`'s motion cards had the same failure independently via a server-rendered inline `opacity: 0`. Closed in plan 1 task 11.                                                                                                                                                     |
| B21 | `src/theme/ThemeProvider.tsx`               | The prescribed B4 fix (derive initial state from the DOM class) traded a one-frame wrong icon for a hydration mismatch — React error #418 on every load for a light-theme visitor, discarding the server HTML. Closed in plan 1 task 14 by removing theme-dependent markup entirely.                                                                                                                                                                                          |
| B22 | `src/app/(site)/[locale]/fonts.ts`          | next/font's generated `'Tajawal Fallback'` uses `src: local(Arial)`, and Arial has no Arabic glyphs, so Arabic fell through to an unmeasured system font. Measured mismatch: `size-adjust` 143.30%. Produced CLS 0.059 and an LCP regression on `/ar` in production. Closed in plan 2 task 2.                                                                                                                                                                                 |
| B23 | `netlify.toml`                              | The B12 header set was declared where it could not reach HTML documents: netlify.toml headers apply to files the CDN serves, and pages come from the Next runtime's function. Static assets carried the full set; every page carried none. Closed in plan 2 task 1 by moving them to `next.config.mjs`.                                                                                                                                                                       |
| B24 | `next.config.mjs`                           | **The CSP from B23 broke `npm run dev`.** React's development build and Turbopack's HMR client both evaluate code at runtime, so `script-src` without `'unsafe-eval'` killed every page with "eval() is not supported in this environment". HMR also needs `ws:` in `connect-src`. Both are now gated on `NODE_ENV === 'development'`; production keeps the strict policy. Closed in plan 2.                                                                                  |
| B25 | `src/lib/auth/password.ts`                  | `@next/env` runs dotenv-expand over every `.env` file, and an argon2 hash is `$argon2id$v=19$m=…` — every `$name` expands to nothing. Plain, double-quoted, single-quoted and backslash-escaped values are all destroyed. Netlify's dashboard does no expansion, so a raw hash works in production while every local login fails. `ADMIN_PASSWORD_HASH` now holds base64. Closed in plan 2 task 5.                                                                            |
| B27 | `src/lib/content/blobs.ts`                  | The bundled-data fallback logged `console.error` when Netlify Blobs was absent. Next 16's dev overlay promotes a server-side `console.error` into a full-screen error, so `npm run dev` threw MissingBlobsEnvironmentError on every page load while the fallback was working exactly as designed — the reporting level was lying about the outcome. Absence of a Blobs runtime is now a one-time `console.warn`; genuine failures still log at error level. Closed in plan 2. |
| B26 | `src/app/(admin)/admin/layout.tsx`          | The admin set no font variables, so Tailwind's `font-sans` resolved through `var(--font-sans, var(--font-arabic))` — both undefined in that tree — and fell to the browser serif default. Then fixing it with `preload: true` leaked the admin's Inter into the **public** Arabic route's preloads. Closed in plan 2 task 10.                                                                                                                                                 |
