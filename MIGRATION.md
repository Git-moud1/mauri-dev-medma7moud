...
## 12. WITHDRAWN: hero concepts A1, A2 and A3 were all removed

Three animated hero concepts were built and measured against each other, and then
**all three were deleted at the owner's request** — including A1, which had
shipped. The hero is now the server-rendered headline, CTAs and stats over CSS
grain: no canvas, no capability probe, no concept switch.

Nothing is lost that a checkout cannot restore. The concepts and their full
measurement writeup are at:

- `a6a2996` — A3 built alongside A1 and A2, with its harness and assets
- `fbe2e2a` — every figure re-measured against the deploy preview

What the removal took with it, so the next hero does not have to rediscover it:

- **`three` and `@react-three/fiber` are out of `package.json`.** They existed
  only for A2 and A3. Anything that brings a 3D hero back brings them back too,
  and with them the numbers below.
- `?hero=` no longer selects anything. The query param is inert.
- `hero.scrollHint` is gone from all three dictionaries.
- `scripts/gen-hero-depth.mjs`, `scripts/measure-hero.mjs`, `public/hero/*` and
  `tests/hero.spec.ts` are deleted.

### The one thing worth carrying forward

The three concepts were separated by two orders of magnitude in cost. Measured on
the deploy preview at `fbe2e2a` — nine runs per cell, 390x844, 4x CPU throttle,
slow 4G, against a pre-hero control on the same URL:

| Concept             | JS beyond first-load | TBT over control, `/ar` |
| ------------------- | -------------------- | ----------------------- |
| A1 (raw GLSL)       | 3.1 KB               | +106 ms                 |
| A2 (three.js mesh)  | 231.2 KB             | +1365 ms                |
| A3 (`three/webgpu`) | 414.1 KB             | +2347 ms                |

First-load JS was **identical at 200.0 KB in all three cases**, because every
concept sat behind a `next/dynamic` boundary. That part of the architecture worked
and is worth reusing whatever replaces it: the entry chunk never grew, and the
whole cost difference is in what the boundary went on to load.

**`three/webgpu` cannot be made cheap on this stack.** Its node and backend layer
is 413.6 KB gzipped before a line of application code, on top of the
`three.core.js` it shares with `three`. Tree-shaking does not help much, because
the renderer's node builders reach most of the node library. If a future hero
wants real 3D, that is the entry fee — and TBT is the metric this site is already
worst at.

A quieter finding, also worth keeping: a hero whose background is a **photograph**
cannot follow the theme. A3's backdrop had charcoal baked into it, so on the light
palette the near-black `--fg` headline sat on a dark image and stopped being
readable. A1 was procedural and re-read `--bg` live, so it themed for free. Any
image-based hero has this problem and has to answer it.

---

## 13. Plan 3 — the SEO surface: icon, sitemap, share cards, structured data

The four gaps the brief's Phase 1 and Phase 2 still had open are closed. All of
it is verified against a served response, not against a config file — the new
`tests/seo.spec.ts` is the gate, and it caught one real bug before the work
shipped (see the merge rule below).

### What landed

- **Tab icon.** `src/app/icon.png` (256) and `src/app/apple-icon.png` (180),
  generated by `scripts/gen-icons.mjs` from `public/mauri-dev.jpeg`. The script
  measures the M mark's bounding box by luminance and re-centres it on black
  rather than cropping a square out of the source, which would drag arcs of the
  logo's ring into the corners. Re-run it if the logo is ever replaced.
- **`src/app/sitemap.ts`.** Six URLs — three locales times the homepage and the
  catalogue — each carrying the same `hreflang` alternates the pages emit.
  `robots.ts` has advertised `/sitemap.xml` since plan 2; until now that URL
  404'd. No `lastModified`: content is editable from the admin without a
  redeploy, so a build-time date would be a freshness claim the deploy cannot
  make.
- **Share cards.** `opengraph-image.tsx` under `[locale]`, mirrored by
  `twitter-image.tsx`, both re-exported into `projects/`. One 1200x630 card per
  locale, in that locale's own headline, prerendered at build time.
- **Structured data.** `src/lib/jsonld.ts` builds one `@graph` — `Person`,
  `WebSite`, `ProfessionalService` with `areaServed`, and an `ItemList` of
  `CreativeWork` — rendered by `src/components/JsonLd.tsx`. `sameAs` comes from
  the admin's social settings, so clearing a platform removes it from the graph
  on the next revalidation, exactly as it removes it from the footer.
- **`GOOGLE_SITE_VERIFICATION`.** Optional env var, documented in
  `.env.example`. Unset, no tag is emitted at all.

### Three things that cost time, so they do not cost it twice

**Metadata merges one level deep, and that takes the images with it.** A page
that declares `openGraph` at all replaces its layout's block entirely — the
`opengraph-image` convention's entries included. `/[locale]/projects` declares
one, so it went out with `og:title` and **no `og:image`**: the bare share link
the brief exists to fix, on the page most likely to be shared. The fix is the
two re-export files under `projects/`. `src/lib/metadata.ts` is where the whole
block is spelled once so the three callers cannot drift.

**Satori does not implement the bidirectional algorithm.** It shapes Arabic
correctly — the letters join — and then lays the words out left to right, so
`تُنمّي الأعمال` rendered as `الأعمال تُنمّي`, the sentence backwards.
`direction: 'rtl'` changes nothing; it is not implemented either. Both were
rendered and looked at. The card therefore expresses word order as layout: one
span per word in a `row-reverse` flex row, which is what `Line` in
`opengraph-image.tsx` does.

**Satori trims every style value it is handed.** `maxWidth: rtl ? undefined :
1010` throws `Cannot read properties of undefined (reading 'trim')` and the
image route 500s — the tag stays in the HTML and the card silently never
renders. A conditional property has to be spread in, not set to `undefined`.

Also worth knowing: the OG fonts are **TrueType copies in `assets/fonts/`**,
outside `public/`. Satori cannot read woff2, which is the only format
`public/fonts` holds, so the two faces the card draws with are vendored a second
time — 179 KB that never reaches a browser. See `assets/fonts/README.md`.

### Verification at this commit

`tsc`, `eslint` and `next build` clean; Playwright **254 passed / 0 failed / 14
skipped** (24 of those are the new SEO suite). `/[locale]`,
`/[locale]/projects` and all four image routes prerender as SSG — the CDN HTML
the architecture depends on is intact.
